# -------------------------------------------------  This tutorial starts at 370 line
...
# queremos mostrar quando é o mes de maior temperatura ou menor, ter ao inves do número como no sapply termos o nome em uma matrix
...
which.max(Chicaco[1,]) # mostra qual mes é o maior na linha 1 de chicago

names(which.max(Chicaco[1,])) # colocando o nome conseguimos extrair apenas o nome do mes

# precisamos utilizar as funções apply para trazer o resultado que queremos


apply(Chicaco,1,function(x) names(which.max(x)))  




# agora iremos tentar aplicar a função acima a todos os componentes da lista weather



sapply(Weather,function(y)apply(y,1,function(x) names(which.max(x))))

#função sapply(lista em que será aplicada, função y faz aplicar tudo em cada coisa da lista,apos isso colocamos a função x que conseguimos fazer anteriormente)
